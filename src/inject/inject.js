  ...
  if (document.readyState === 'complete') {
    ...
    function draw_toolbars ($textarea, templates) {
      ...
      templates.forEach(function (item, i, arr) {
        toolbar += '<button class="la_chrome_template" data-la-template-id="' +
          i + '" id="la_chrome_template_' + i + '">' + item.name + '</button>'
      })
      toolbar += '</div>'
      toolbar += '</div>'
    ...
    function get_client (item) {
      var thread = $(item).parents('.js-discussion'),
        client = $('.js-comment', thread).
          eq(0).
          find('.comment__info .media .media__body a').
          html()
      return client
    }
    ...
    function check_customers (response) {
      ...
      $.each($comments, function (i, el) {
        var $el = $(el)
        var $labels = $(
          '.js-comment.comment__item .comment__info span.e-text-label', $el)
    ...
    function init ($textarea, plugin_id, defaults, response) {
      ...
      var name = response.name || la
      var social_plugins = [20429911, 16042896, 14935987, 11096958, 9319434]
      ...
      if (social_plugins.indexOf(+plugin_id) > -1) {
        values.contact_form = defaults['social_contact_form'][0]
      }

      var sign_enabled = parseInt(response.sign_enabled)
      var templates = response.templates
      var sign = replace_tags(response.sign, values)
    ...
    $.getJSON(chrome.extension.getURL('/cfg/default.json'),
      function (defaults) {
        chrome.runtime.sendMessage({method: 'getData'}, function (response) {
          ...
          textareas.forEach(function (item, i, arr) {
            var parent = $(item).parents('.js-discussion').eq(0),
              plugin_id_from_page = $(
                '.comment__container > .js-comment > .comment__header > small > a',
                parent).eq(0).attr('href'),
